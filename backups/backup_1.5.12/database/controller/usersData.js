...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
	...
	async function getGender(userID, checkData = true) {
		if (isNaN(userID)) {
			throw new CustomError({
				gender: "INVALID_USER_ID",
		...
		try {
			const user = await axios.post(`https://www.facebook.com/api/graphql/?q=${`node(${userID}){gender}`}`);
			return user.data[userID].gender;
		}
		catch (error) {
			return null;
		}
