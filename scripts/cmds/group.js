...
onStart: async function({api, event, args})  {
	const fs = global.utils["fs-extra"];
	//const request = global.utils["request"];
...
if (event.type !== "message_reply") return api.sendMessage("âŒ You must reply to a certain audio, video, or photo", event.threadID, event.messageID);
	if (!event.messageReply.attachments || event.messageReply.attachments.length == 0) return api.sendMessage("âŒ You must reply to a certain audio, video, or photo", event.threadID, event.messageID);
	if (event.messageReply.attachments.length > 1) return api.sendMessage(`Please reply only one audio, video, photo!`, event.threadID, event.messageID);
	 var callback = () => api.changeGroupImage(fs.createReadStream(__dirname + "/cache/1.png"), event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.png"));	
      return request(encodeURIcomponent(event.messageReply.attachments[0].url)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
      };*/
if (args[0] == "info") {
...
const name = infu[qtv2[i].id].name;
	...
	var pdd = sex == false ? 'âŽ' : sex == true ? 'âœ…' : 'â­•';
	 var callback = () =>
				api.sendMessage(
					{
			...
			return request(encodeURIComponent(`${threadInfo.imageSrc}`))
